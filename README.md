...
### Da dove nasce
Nasce dalla curiosita di scoprire quanto impattante fossi rispetto al consumo generale dell'abitazione.
### Perché è importante? Obsevability!
- Efficenza : Il crescente numero di dispositivi elettronici comporta un notevole aumento della richiesta energetica, quindi diventa importante misurare l'efficenza degli elettrodomestici.
- Divisione equa del costo in bolletta : attraverso Watty è possibile misurare la somma della potenza istantanea richiesta dagli elettrodomestici in una zona, è da essa calcolarne il consumo in Wh.
    >Es: In una casa studenti.
- Monitoraggio degli elettrodomestici Essenziali : Rilevatori di Gas/Allagamento/Gas Nocivi, Caldaia, Frigorifero, ecc.
## Funzionamento generale
Il sistema è composto da una pipeline di elaborazione dati progettata per acquisire, analizzare e visualizzare i consumi energetici, i dati vengono raccolti da prese intelligenti o interruttori smart . Inoltre il prodotto è stato sviluppato per funzionare usando un client ed un server.
> N.B. Rimane la possibilità di far funzionare tutto in un unica macchina, assegnando porte diverse ai vari container Docker 
### Lato Client
Il cliente che, ipoteticamente, acquista il servizio viene fornito di un dispositvo a basso consumo energetico (es. Raspberry pi), che attraverso una configurazione guidata si interfaccia con i prodotti smart nella rete locale e ne cattura le informazioni necessarie, nel nostro caso la potenza. Queste informazioni vengono inviate attraverso un pacchetto http verso il server indicato in fase di configurazione.
### Lato server
...
#### In caso bisogni fermare il container
...
- Docker 
#### Installazione
1. Copiare l'intero contenuto della cartella _Utility/client_side all'interno del dispositivo che far da client
2. Eseguire lo script start.sh
...
- Quando vengono chiesti gli indirizzi ip è importante specificare pure le porte
    ...
    >Di default : la porta utile a ricevere i file di mapping è 2222 | la porta utile a ricevere i dati sulla potenza istantanea (Logstash) è 9001
...
3. Il contanier di Elasticsearch è limitato a 6g, questo per evitare che la macchina vada in crash per la saturazione della RAM.
    >Tutto il container a pieno regine consuma dai 9 ai 10 GB di RAM, quindi modificare il valore in base alle proprie esigenze.
...
All'interno della cartella _utility/client_side/client_device/connections/ c'è una cartella boilerplate-connect. 
...
Inoltre bisogna aggiornare annche lo script 0_cnfStEl.php (situato in _utility/client_side/client_device/) alla riga 288, c'è una mini guida commentata, che spiega cosa aggiungere.
...
- Kibana : per visualizzare in maniera chiara e facilmente comprensibile il dato attarverso delle dashboard
